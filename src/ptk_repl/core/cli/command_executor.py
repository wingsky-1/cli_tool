...
class CommandExecutor:
    ...
    现在使用 ICliContext 接口，提供类型安全的上下文传递。
    """
    ...
    def execute(self, command_str: str) -> None:
        ...
        if cmd_info:
            ...
            if getattr(handler, "_is_typed_wrapper", False):
                ...
                handler(self.cli_context, remaining)
            else:
                # 普通命令处理
                handler(remaining)
        else:
            ...
            if len(tokens) == 1:
                ...
                self.cli_context.perror(f"未知命令: {tokens[0]}")
    ...
    def _handle_module_only(self, module_name: str) -> None:
        ...
        if module:
            ...
            self.cli_context.poutput(f"{module.name} 模块 - {module.description}")
            self.cli_context.poutput("\n可用命令:")
            for cmd in commands:
                if module.name == "core":
                    full_cmd = cmd
                else:
                    full_cmd = f"{module.name} {cmd}"
                self.cli_context.poutput(f"  • {full_cmd}")
            return
        ...
        self.cli_context.perror(f"未知命令: {module_name}")
