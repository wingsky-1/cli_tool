...
name: deep-plan
description: |
  深度规划助手 - 适用于复杂架构分析和多维度规划任务

  使用场景（满足任一即触发）：
  • 架构分析/重构规划（涉及多模块）
  • 多维度任务（功能+性能+安全等 ≥2 个维度）
  • 需要深度代码探索和并行分析
  • 需求模糊需要大量澄清
  ...
  与 plan mode 的区别：
  • plan mode：线性流程，适合简单-中等任务
  • deep-plan：多 agent 协作，适合复杂多维度任务

  不适用场景（使用 plan mode 或直接执行）：
  • 单文件修改 • 明确的小功能 • 单一维度任务
...
**触发关键词**：
"分析架构" / "重构" / "设计方案" / "改进/优化方向" / "思考/探讨"

**维度列表**：功能 • 性能 • 安全 • 测试 • 重构 • 文档

**快速判断**：
- "分析/设计/重构/优化" + 多模块 → deep-plan
- "添加/修复/更新" + 单文件 → 直接执行
- 介于两者之间 → plan mode
...
## 何时使用
```
用户请求 → 深度代码探索？+ ≥2维度？+ 多Agent协作？ → deep-plan ⭐
```

## 核心流程（6 步）
1. **需求澄清**（Phase 1）：确认所有模糊点
2. **场景识别**（Phase 2）：判断维度和策略
3. **多 Agent 协作**（Phase 3）：并行分析各维度
4. **代码探索与评估**（Phase 4）：深度分析
5. **计划生成**（Phase 5）：任务规划和依赖分析
6. **验证输出**（Phase 6-7）：质量检查 + 结构化输出

## 关键原则（3 条）
- ✅ **渐进式发现**：描述任务，让框架自动选工具
- ✅ **澄清优先**：模糊点必须确认后再分析
- ✅ **多 Agent 协作**：多维度任务自动拆分并行

---

<details>
<summary>📖 实践指南（点击展开，50 行内）</summary>
...
| 维度 | Agent 描述 | 关注点（3-5 个） | 输出 |
|------|-----------|-----------------|------|
| 架构设计 | 专注于架构设计，分析接口定义、模块划分、依赖关系 | 接口完整性、模块边界、依赖合理性 | 架构图、接口清单、依赖树 |
| 性能优化 | 分析性能瓶颈，包括算法复杂度、资源使用、并发处理 | 复杂度、资源占用、并发安全 | 瓶颈清单、优化建议、性能指标 |
| 安全加固 | 识别安全漏洞，包括输入验证、权限控制、数据保护 | 注入风险、权限漏洞、数据泄露 | 风险清单、加固方案、测试用例 |

### 并行执行
单条消息启动多个 agent（不超过10个），各自聚焦特定维度，等待全部完成后汇总。
...
|---------|---------|---------|---------|---------|
| **需求理解错误** | 用户反馈偏离预期<br>/"这不是我想要的" | 立即澄清，重新开始 Phase 1 | 丢弃当前计划 | Phase 1 批量确认所有关键点 |
| **Agent 冲突** | 不同 Agent 建议矛盾<br>/(方案 A vs 方案 B) | 启动冲突解决流程<br>权衡优劣 | 选择最优方案 | 提前明确优先级和约束 |
| **代码探索失败** | 无法找到相关符号<br>/工具超时或异常 | 切换探索策略<br>/(符号→文本→人工) | 回退到上一步骤 | 多种探索方法并行 |
| **依赖分析错误** | 循环依赖<br>/关键路径不明确 | 重新梳理任务关系<br>拆分任务 | 合并任务或重新排序 | Phase 5 验证 DAG 性质 |
| **验证失败** | L1/L2 检查未通过<br>/(位置不明/依赖不清) | 补充分析或重新生成 | 退回 Phase 4 补充 | Phase 6 强制检查点 |
| **执行中变更** | 用户中途修改需求 | 评估影响范围<br>增量更新计划 | 基于最近 Checkpoint | 设置频繁 Checkpoint |
...
**恢复示例**：
```markdown
[CHECKPOINT] 阶段 2 完成：多 Agent 分析汇总
